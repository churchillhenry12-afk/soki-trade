...
- The bridge must pass its protocol/health check before its configuration is
  saved.
- The bridge must report `account_mode=DEMO`. `LIVE` and `UNKNOWN` are both
  rejected.
...
1. Log into a dedicated demo account in the MetaTrader 5 terminal.
2. Run a compatible REST or MCP bridge on the terminal host.
3. Open **Connections** or type `/connect mt5`.
4. Enter the bridge transport, endpoint, and bearer token.
5. Let Soki Trade verify the protocol and confirm that the account is DEMO.
...
Until that companion service is connected and reports a DEMO account, MT5
account data remains unavailable and the UI keeps the gateway visibly
disconnected.
